...
Пример запуска с указанием параметров (эти значения уже зашиты как дефолтные, поэтому их указывать не обязательно):

```bash
PYTHONPATH=. .venv/bin/python src/interview_assistant/main.py \
  --backend=agent-builder \
  --project-id="project-7d570aed-312f-4939-9a4" \
  --location="eu" \
  --app-id="289f0946-709f-4a80-b7ff-e863aace6bde" \
  --version-id="2abf9851-9b93-405d-8420-2f73931def9a" \
  --deployment-id="17540902-bba7-4693-b05d-52c77970c493" \
  --session-id="aAOXjF"
```
...
```bash
PYTHONPATH=. .venv/bin/python src/interview_assistant/main.py --backend=agent-builder
```
